function[Spots] = segmentSpots_JF(G, Threshold, Shadows, XYRes)
...
microscope = 'LIFExport';

%zSize = FrameInfo(1).NumberSlices + 2;
zSize = size(G,3);
numFrames = size(G,4);

%OutputFolder1=[Path,File,Name,'dogs'];
%mkdir(OutputFolder1) 
...
% remove so that everytime it does DoG and segmentation
%if justDoG
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         h=waitbar(0,'Generating DoG images')        
 ...
 %           disp(['dog f',num2str(current_frame)])
...
%                     dog_name = ['DOG_',iIndex(current_frame,3),'_z',iIndex(i,2),'.tif'];
...
%             else 
                ...
                %for i = 1:zSize 
                    im = G(:,:,i);
                    %im = double(imread([PreProcPath,filesep,Prefix,filesep,Prefix,'_',iIndex(current_frame,3),'_z',iIndex(i,2),nameSuffix,'.tif']));
                    dogZ(:,:,i) = filterImage(im,filterType,sigmas, filterSize);
                    %dog_name = ['DOG_',iIndex(current_frame,3),'_z',iIndex(i,2),'.tif'];
                    %imwrite(uint16(dog), [OutputFolder1,filesep,dog_name])
                end
            %end
...
%else       
    ...
    %    h=waitbar(0,'Segmenting spots');
     %   Threshold = thresh(q);
     %   for current_frame = 1:numFrames
      %      disp(['finding spots f',num2str(current_frame)])
       ...
       %     G = ReadSingleStack(reader,Channels,Slices,Frames0,Flip,From,1,f);
            for i = 1:zSize  
                ...
                %im = double(imread([PreProcPath,filesep,Prefix,filesep,Prefix,'_',iIndex(current_frame,3),'_z',iIndex(i,2),nameSuffix,'.tif']));
%                 try
%                     dog = double(imread([OutputFolder1, filesep,'DOG_',iIndex(current_frame,3),'_z',iIndex(i,2),'.tif']));
%                 catch
...
%%
Spots{1} = struct;
%Spots{1}(1).Fits(1) = struct;
if numel(fieldnames(Particles)) ~= 0
        %z-tracking
        numFrames = 1;
        changes = 1;
        while changes ~= 0
        ...
        for i = 1:length(Particles)
            ...
            Particles(i).CentX = round(mean([Particles(i).xDoG]));
            Particles(i).CentY = round(mean([Particles(i).yDoG]));
            Particles(i).MaxInt = Particles(i).FixedAreaIntensity(max_index);
